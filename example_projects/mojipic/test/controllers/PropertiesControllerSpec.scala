...
class PropertiesControllerSpec extends PlaySpec with MockitoSugar {
  ...
  trait Setup {
    ...
    val sut = new PropertiesController(mockedGetPicturePropertiesService, ExecutionContext.global)
  }

  "PropertiesControllerSpec#getAll" should {
    "return picture properties" in new Setup {
      val localDateTime = LocalDateTime.now(clock)
      when(mockedGetPicturePropertiesService.getAll(localDateTime)).thenReturn(Future.successful(Seq(property)))
      ...
      val actual = sut.getAll(Some(localDateTime.toString))(FakeRequest())
      assert(contentAsJson(actual) === expected)
      assert(contentType(actual) === Some("application/json"))
      verify(mockedGetPicturePropertiesService, times(1)).getAll(localDateTime)
    }

    "return picture properties even if a last created time is not specified " in new Setup {
      when(mockedGetPicturePropertiesService.getAll(LocalDateTime.parse("0000-01-01T00:00:00"))).thenReturn(Future.successful(Seq(property)))
      val expected = Json.parse("""[{"id":"123","value":{"status":"Converting","twitterId":"456","fileName":"randy.jpg","contentType":"image/jpeg","overlayText":"Randy","overlayTextSize":25,"createdTime":"1991-03-31T09:00:00"}}]""")
      val actual = sut.getAll(None)(FakeRequest())
      assert(contentAsJson(actual) === expected)
      assert(contentType(actual) === Some("application/json"))
      verify(mockedGetPicturePropertiesService, times(1)).getAll(LocalDateTime.parse("0000-01-01T00:00:00"))
    }

    "return InternalServerError if GetPicturePropertiesService returns DatabaseException" in new Setup {
      val localDateTime = LocalDateTime.now(clock)
      when(mockedGetPicturePropertiesService.getAll(localDateTime)).thenReturn(Future.failed(DatabaseException("")))
      val actual = sut.getAll(Some(localDateTime.toString))(FakeRequest())
